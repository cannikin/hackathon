class RewardsController < ApplicationController
  ...
  # GET /rewards/new.xml
  def new
    @reward = Reward.new(:device => current_user.business.devices.first)
  ...
  def create
    ...
    respond_to do |format|
      if @reward.save
        format.html { redirect_to(business_path, :notice => 'Reward was created!') }
  ...
  def update
    ...
    respond_to do |format|
      if @reward.update_attributes(params[:reward])
        format.html { redirect_to(business_path, :notice => 'Reward was successfully updated.') }
        format.xml  { head :ok }
      else
